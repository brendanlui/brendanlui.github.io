...
if(!isset($_SESSION['username']) || empty($_SESSION['username']) || strcmp($_SESSION['username'], 'admin_brendanlui') != 0){
  ...
  exit;
	...
	if(time() - $_SESSION['timestamp'] > 900) { //subtract new timestamp from the old one //900
	...
	$sql = "UPDATE database_admins SET last_activity = '".$date."' WHERE username = '".$_SESSION['username']."'";
	...
	$sql = "SELECT * FROM database_admins WHERE username = '".$_SESSION['username']."'";
	$result = mysqli_query($conn, $sql);
	
	while($row = mysqli_fetch_array($result))	//save object_mobile
	{
		if (!(strpos($row['ip_address_used'], $user_ip) !== false)) {
			$sql = "UPDATE database_admins SET ip_address_used = '".$row['ip_address_used'].",".$user_ip."' WHERE username = '".$_SESSION['username']."'";
