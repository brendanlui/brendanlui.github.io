...
if($_POST){
	...
	$sql = "UPDATE database_record SET user_do_this_job = 'none' WHERE id = ".$id."";
	...
	$result = mysqli_query($conn, $sql);
	while($row = mysqli_fetch_array($result))	//save title
	{
		$GLOBALS['title'] = $row['title'];
	}
	$sql = "UPDATE database_record SET resolved = 'Available' WHERE id = ".$id."";
...
////////////
	...
	//insert in database_notice
		...
		('".$_SESSION['username']."', '".$subject_mobile."', 'changed', '".$save_receiver."', '".$object_mobile."', '".$title."', '".$date."', 'Unread', '".$id."')
		";
	...
	echo '
		<div class="success_reply">Receiver is none now.</div></br>
	';
				echo '
	...
	header("location: logined_next.php");
}
?>
